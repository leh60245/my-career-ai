        ...
        with search_form_column, st.form(key="search_form"):
            ...
            if submit_button and st.session_state["page3_write_article_state"] in ["not started", "show results"]:
                ...
                if not pass_appropriateness_check:
                    ...
                    alert = st.warning(st.session_state["page3_warning_message"], icon="[WARNING]")
                    time.sleep(5)
                    alert.empty()
                else:
